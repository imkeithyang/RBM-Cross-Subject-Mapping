    ...
    for omega in omegas:
        ...
        for Threshold in Thresholds:
            ...
            for h_dim in no_HiddenUnits:
                ...
                Rate_LDA, Rate_MLRBM, Rate_FRBM, Rate_NO = {}, {}, {}, {}
                for index in Z:
                    Rate_LDA['moth_{}'.format(index)] = np.zeros((MCT))
                    for other_index in Z:
                        if other_index != index:
                            Rate_MLRBM['moth_S_is_{}_D_is_{}'.format(index,other_index)] = np.zeros((E, MCT))
                            Rate_FRBM['moth_S_is_{}_D_is_{}'.format(index,other_index)] = np.zeros((E, MCT))
                ...
                for lr in learning_rates['both_MLandF']:
                    ...
                    for m in range(MCT):
                        ...
                        for index in Z:
                            ...
                            Rate_LDA['moth_{}'.format(index)][m], _, clfs_Z['clf_moth-{}'.format(index)] = TrainTest_LinearDecoder( clfs_Z['clf_moth-{}'.format(index)], signals_Z_train[:,P_Z['moth-{}'.format(index)][0]:P_Z['moth-{}'.format(index)][1]+1], np.expand_dims(targets_Z_train[:,0],axis=1), signals_Z_test[:,P_Z['moth-{}'.format(index)][0]:P_Z['moth-{}'.format(index)][1]+1], np.expand_dims(targets_Z_test[:,0],axis=1), mode='train' )
                        ...
                        for epoch in range(E):
                            ...
                            for index in Z:
                                ...
                                signals_Z_test_init_temp = signals_Z_test.copy()
                                signals_Z_test_init = np.random.randn(signals_Z_test_init_temp.shape[0], signals_Z_test_init_temp.shape[1])
                                ...
                                if epoch % (E//20) == 0:
                                    other_index = random.choice(Z)
                                    while index == other_index:
                                        other_index = random.choice(Z)
                                    print( '  (Y={},sigma={},omega={},Th={},h_dim={},lr={}); (m={},Epch: {}); X->Y(CD/F): {:.4f}/{:.4f} . . .'.format(index,sigma,omega,Threshold,h_dim,lr,m,epoch,Rate_MLRBM['moth_S_is_{}_D_is_{}'.format(index,other_index)][epoch,m],Rate_FRBM['moth_S_is_{}_D_is_{}'.format(index,other_index)][epoch,m]) )
                            if epoch % (E//20) == 0:
                                print('-'*50)
                    ...
                    savemat('Results/SCENARIO_II_F_OneMissingSubj_sigma{}_PCAThreshold{}_noHidUnit{}_lr{}_tol{}_TrainTestRatio{}.mat'.format(sigma,Threshold,h_dim,lr,init_tol,omega),
                            ...
                            'fs':fs,'duration':duration,
                            'E':E,'batch_size':batch_size,'learning_rates':learning_rates,'weight_decay':weight_decay,'step_size':step_size,'gamma':gamma,
                            'Rate_LDA':Rate_LDA,'Rate_FRBM':Rate_FRBM})
